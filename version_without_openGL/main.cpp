...
void RayMarch(std::unique_ptr<Scene> scene) {
  ...
  for (auto &pixel : pixeles_pos) {
    ...
    if (ray_hit == nullptr)
      ...
      img.pixels[i].color =
          scene->objects[ray_hit->object_indice]
              ->GetTextureElements(Point3(std::move(ray_hit->point)))
              ->color;
...
int main(int argc, char const *argv[]) {
  ...
  std::unique_ptr<Uniform_Texture> RedTexture =
      std::make_unique<Uniform_Texture>(255, 0, 0, 0.8, 3);
  std::unique_ptr<Sphere> s1 =
      std::make_unique<Sphere>(Point3(3, 0, 0), 1, std::move(RedTexture));

  std::unique_ptr<Uniform_Texture> texture2 =
      std::make_unique<Uniform_Texture>(20, 200, 40, 1, 1);
  std::unique_ptr<Sphere> s2 =
      std::make_unique<Sphere>(Point3(5, 2, 1), 1, std::move(texture2));

  std::unique_ptr<Point_Light> p_l =
      std::make_unique<Point_Light>(1, 1, 1, ColorRGB(255, 255, 255), 1);
  scene->addObject(std::move(s1));
  scene->addObject(std::move(s2));
  scene->addCamera(Point3(0, 0, 0), Point3(1, 0, 0), Vector3(0, 1, 0), 45, 45,
                   1);
